...
class Kue {
  ...
  dispatch(
    key,
    data,
    { priority = 'normal', attempts = 5, remove = true, jobFn = () => {} } = {}
  ) {
    if (typeof key !== 'string') {
      throw new Error(
        `Expected job key to be of type string but got <${typeof key}>.`
      );
    }
    ...
    job.save(err => {
      if (err) {
        this.Logger.transport('file').error(
          'An error has occurred while creating a Kue job.'
        );
        throw err;
      }
    });
    ...
    job.on('failed', errorMessage => {
      this.Logger.transport('file').error(
        `fail to send sms after ${attempts} time to user ${
          data.to
        } => err:${errorMessage}`
      );
    });
  ...
  listen() {
    this.jobs.forEach(link => {
      ...
      if (typeof Job.concurrency !== 'number') {
        throw new Error(
          `Job concurrency value must be a number but instead it is: <${typeof Job.concurrency}>`
        );
      }
      ...
      this.instance.process(Job.key, Job.concurrency, (job, done) => {
        jobInstance
          ...
          .catch(error => {
            this.Logger.error(
              `Error processing job. type=${job.type} id=${job.id}`
            );
            done(error);
          });
      });
    ...
    this.Logger.info(
      `kue worker listening for ${this.registeredJobs.length} job(s)`
    );
  }
}
